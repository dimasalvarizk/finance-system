...
export const addInvoicePayment = async (invoiceNo: string, paymentData: { amount: number; currency?: string; exchangeRate?: number; paymentDate: string; note?: string; proofUrl?: string; saveOverpaymentCredit?: boolean; companyCode?: string }) => {
...
export const getPaymentReceipt = async (invoiceNo: string, paymentId: string) => {
  const response = await invoiceAPI.get(`/${invoiceNo}/payments/${paymentId}/receipt`);
  return response.data;
};

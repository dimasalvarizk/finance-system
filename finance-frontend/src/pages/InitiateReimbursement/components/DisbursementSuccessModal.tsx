...
import type { ReimbursementSuccessState } from '../types';

interface DisbursementSuccessModalProps {
  data: ReimbursementSuccessState | null;
  onClose: () => void;
  formatAmount: (num: number, curr?: string) => string;
}

export const DisbursementSuccessModal: React.FC<DisbursementSuccessModalProps> = ({
  data,
  onClose,
  formatAmount
}) => {
      ...
      >
        ...
        <div className="space-y-2">
          <h3 className="text-[19px] font-bold text-slate-900">
            {t('approvals.bankSuccess') || 'Payment Initiated Successfully!'}
          </h3>
          <p className="text-[13px] text-slate-500 leading-relaxed">
            Reimbursement payout of <span className="font-extrabold text-[#2563eb]">{formatAmount(data.amount, data.currency)}</span> has been dispatched to <span className="font-bold text-slate-800">{data.employeeName}</span> ({data.bankName} - {data.accountNumber}).
          </p>
          <div className="p-3 bg-slate-50 rounded-xl border border-slate-100 text-[11.5px] font-mono text-slate-600">
            Ref: <span className="font-bold text-slate-900">{data.transactionRef}</span>
          </div>
        </div>
        ...
        <div className="pt-2 flex flex-col sm:flex-row items-center justify-center gap-2.5">
          ...
          >
            {t('requests.backToListing') || 'Back to Approvals'}
          </button>
          <button
            onClick={() => {
              onClose();
